...
- **明确不做**：独立日程/事件表、团队日历（无创建主体，砍掉；团队维度由热力图/回顾页代表）、提醒推送（等站内通知/Web Push 就绪，不做"无触达的提醒"）、重复规则、iCal 同步、自然语言日期解析、memo `eventAt` 字段。
...
> **R1（反向链接回顾）→ R10（日历视图，可并行：migration 先行）→ R4（多渠道输入）→ R7（附件观测）→ R8-R9（E2E 与生态）**

理由：

- R1 纯前端复用现有 relations，成本最低、立刻有 flomo 味。
- R10 零新表零 migration、纯前端 + 聚合 API，成本最低批次，可与 R1 并行穿插。
