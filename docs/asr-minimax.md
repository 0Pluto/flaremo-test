...
- **Capture flow when MiniMax is the only configured provider**: record → stop → transcribe. The client wraps its existing 16 kHz mono s16le PCM frames into a WAV (44-byte RIFF header) and uploads when the session ends; the UI shows audio-ready / transcribing states. Live partial text stays a streaming-adapter feature; no pseudo-streaming.
