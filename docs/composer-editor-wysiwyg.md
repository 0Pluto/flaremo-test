...
| # | 问题 | 推荐 | 状态 |
|---|---|---|---|
| D1 | 是否保留「markdown 源码切换」给高级用户 | 暂不做；存储是 markdown 本身已是兜底 | 待 Kim 表态 |
| D2 | 工具栏形态 | 保留现有底部单行按钮位（克制），不做浮动气泡菜单 | 待 Kim 表态 |
| D3 | 时间线卡片内 checkbox 点击勾选 | 编辑框先行，卡片端联动涉及 memo 内容回写，另立决策 | 后置 |
