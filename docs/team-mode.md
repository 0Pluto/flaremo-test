...
- 读者侧可读写自己的个人空间，团队空间为只读视图（不显示发布入口与编辑操作）。

管理（账户页面 → 团队）：

- 设为读者 / 续期（从 `max(当前, 现到期日)` 顺延 30 或 365 天预设）；
- 撤销读者（删除成员关系，团队空间随之消失）；
- owner 与 admin 不能被降级为读者。

### 机器开通接口

外部系统（脚本、支付 webhook、机器人）通过 PAT 调用管理端点。PAT 与浏览器会话拥有完全相同的权限边界：**凭证持有人本人的角色决定其能力**——实例 owner 的 PAT 可以做实例 owner 能做的一切。

先在账户页面创建 PAT（`memos_pat_` 前缀），然后：
...
- 幂等：重复调用同一邮箱 + 相同到期日得到相同结果（200）；
- 新建账号返回 `created: true`（HTTP 201）与 `activation_path`（一次性激活链接，1 小时有效），把它交给读者即可完成首次登录；已存在账号返回 200；
- 邮箱对应已移出的账号时返回 409；目标是 owner/admin 时返回 403（永不降级）；
- 按用户 ID 的细粒度管理（`PUT/DELETE /api/app/admin/users/:id/reader`、`PATCH /api/app/admin/users/:id/role`）继续可用。
