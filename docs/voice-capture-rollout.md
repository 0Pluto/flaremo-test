...
> 状态：**待 Kim 审批**。基于 2026-09-17 两轮交互审计（录音页 8 节 + 周边链路 7 节，实锤见附录 A）。
...
| P1 | 录音页质感（波形/大圆钮/动效） | web | 无（可与 P0 并行） |
...
- 暂停/续录（见 D1）、打字机逐字效果（与逐句转写节奏不符）、重录按钮（Discard→重开已够）。
...
| # | 决策 | 推荐 | 理由 |
|---|---|---|---|
| D1 | 暂停/续录 | **v1 不做** | 状态机加 pause + 续录合并流复杂度中等但收益存疑；Stop→review→新会话可覆盖多数场景。出现真实需求再排期 |
| D2 | >500s 切片位置 | **客户端** | client 已持有 PCM 帧，零过网浪费；worker 只需校验单片 ≤50MB |
| D3 | 音频格式 | **WAV 原样** | 16k mono ≈ 32MB/h，R2 可承受；opus 转码需引入编码依赖，列为 P4 可选项 |
| D4 | 上传失败降级 | **先保 transcript + 提示** | 语音笔记的价值主体是文字；音频是增强，失败不能拖垮保存 |
| D5 | 时间线卡片 | **徽章+时长 chip，不内嵌播放** | 卡片保持克制；播放体验在详情页音频条已完整 |
| D6 | 保存成功去向 | **保留跳 memo 详情 + toast** | 录完即看到成品是正向反馈；后台列表用 toast 提供路径 |
