...
- **خصوصية كاملة وحذف قابل للتراجع**: المهام مملوكة لصاحبها وحده؛ الحذف يورّد العنصر إلى سلة المهملات حيث يمكن استعادته قبل التنظيف التلقائي.

### 4. التقويم
- **المهام هي المصدر الوحيد للجدول**: يملأ عرض `/calendar` الشهري الأيام الماضية بما كتبتَه والأيام القادمة بما ينتظرك — اسحب لإعادة الجدولة، وأضف مهاماً بتاريخ بسرعة، وتصفّح قائمة الأجندة.
- **تنبيهات التأخر**: المهام المتأخرة تولّد إشعارات داخل التطبيق، مع Web Push اختياري في المتصفح.

### 5. تعاون الفرق وصلاحيات ثلاثية المستويات
...
### 6. أولوية العمل دون إنترنت وتجربة PWA
...
### 9. نظام الإضافات: البطاقات إضافات
- **خمس بطاقات مدمجة**: أبيض، يومية، تذكرة، بطاقة بريدية، وختم بريدي مرسوم بـ canvas.
- **المتجر والإدارة**: من الإعدادات — تصفّح الأدلة، تثبيت بنقرة واحدة (تحقق SHA-256)، تفعيل/تعطيل، الترتيب، البطاقة الافتراضية، الإخفاء. الدليل الرسمي: [flaremo.app/plugins](https://flaremo.app/plugins/registry.json).
- **ارفع إضافاتك**: يمكن للمدير تثبيت حزمة محلية — توجد في هذه النسخة فقط ولا تُرسل إلى أي مكان.
- **أدوات المؤلف**: `pnpm plugin:new` ينشئ الهيكل، و`pnpm plugin:check` يتحقق بـ**نفس القواعد المطبقة عند التثبيت**، و`pnpm plugins:build` يحزم. بطاقات document تخطيط JSON صافٍ؛ وبطاقات sandbox تشغّل HTML/CSS/JS الخاصة بك. راجع [دليل الإضافات](./docs/plugins.md).
- **آمن افتراضيًا**: تعمل البطاقات في صندوق رملي بأصل معتم و**دون أي وصول إلى الشبكة**؛ تبقى حزم المجتمع والعلامات التجارية معطّلة حتى يفعّلها المدير.
