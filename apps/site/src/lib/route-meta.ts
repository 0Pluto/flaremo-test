  ...
  const routes: RouteMeta[] = [
    {
      path: "/",
      locale: "en",
      title: STATIC_PAGE_META.en.homeTitle,
      description: STATIC_PAGE_META.en.homeDesc,
      jsonLd: SOFTWARE_APPLICATION_JSON_LD,
    },
    {
      path: "/docs",
      locale: "en",
      title: STATIC_PAGE_META.en.docsTitle,
      description: STATIC_PAGE_META.en.docsDesc,
    },
  ];

  for (const loc of SUPPORTED_LOCALES) {
    const meta = STATIC_PAGE_META[loc];
    routes.push(
      {
        path: `/${loc}`,
        locale: loc,
...
export function getDocRoutes(): RouteMeta[] {
  ...
  // English default routes at /docs/<slug> and /en/docs/<slug>
  ...
  // Other locales: /zh/docs/<slug>, /ja/docs/<slug>, etc.
  for (const loc of SUPPORTED_LOCALES) {
    if (loc === "en") continue;
    const docLocale = loc === "zh" ? "zh-CN" : "en-US";
    for (const slug of ALL_DOC_SLUGS) {
