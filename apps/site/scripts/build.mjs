...
async function main() {
  ...
  try {
    ...
    // Mirror the plugin store (registry.json + artifacts) so
    // https://<site>/plugins/registry.json is the default directory source
    // for every instance. Generated by `pnpm plugins:build` beforehand.
    const storeDir = path.resolve(root, "..", "..", "plugins", "dist", "store");
    const registrySrc = path.resolve(root, "..", "..", "plugins", "registry.json");
    ...
    if (await exists(storeDir)) {
      const pluginsOut = path.join(outDir, "plugins");
      await cp(storeDir, pluginsOut, { recursive: true });
      await cp(registrySrc, path.join(pluginsOut, "registry.json"));
      console.log("[site] mirrored plugin store → /plugins/");
    } else {
      console.warn(
        "[site] plugins/dist/store missing — run `pnpm plugins:build` first to publish the store",
      );
    }
