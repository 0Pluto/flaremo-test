...
- **AI 네이티브 제2의 뇌**: 내장 MCP 엔드포인트와 Agent Memory 허브를 통해 AI 에이전트(Claude, Cursor, Codex, ChatGPT)가 장기 선호도와 기억 범위를 읽고 갱신할 수 있습니다.
...
### 2. AI 장기 기억 & 네이티브 MCP
- **Agent Memory**: `/memory/mcp` 엔드포인트를 통해 AI 에이전트가 세션을 넘어 이어지는 장기 기억(선호도, 프로젝트 결정, 제약 조건, 교훈)을 기록·갱신합니다.
- **Human in the loop**: `/memory` 화면에서 AI가 기록한 기억을 검토하고, 확인·잠금·수정할 수 있습니다.
- **열린 생태계**: 표준 `/mcp`(Streamable HTTP MCP) 엔드포인트로 프로그래밍 방식의 메모 조회·추가를 지원합니다.
